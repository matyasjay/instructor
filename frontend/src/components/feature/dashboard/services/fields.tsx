...
export default function mapServiceField(
  field: Nullable<ServiceResponse[keyof ServiceResponse]> = Array(0),
  key: string,
) {
  return {
    id: () => field,
    ...
    templates: () =>
      Object(field)
        .toReversed()
        .map(({ id, name, template, description, updatedAt }: Template) => (
          <div className="flex flex-col relative" key={id}>
            <div className="capitalize w-full flex items-center bg-secondary border-x-1 border-white/20 p-2">
              <div className="flex flex-col ml-auto bg-secondary w-full justify-center h-full p-2">
                ...
                <span className="text-xs italic">{`Updated ${dayjs(updatedAt).fromNow()}`}</span>
              </div>
    ...
    users: () =>
      Object(field).map((user: User) => (
        <Badge key={user.id} className="bg-primary/20 px-2 rounded-none ml-2 font-mono">
          {user.name}
        </Badge>
      )),
      ...
      ({
        true: (
          <div className="flex items-center">
            <Button variant="outline" className="w-30 cursor-pointer rounded-none m-0">
              <EyeOffIcon width={20} />
              Private
            </Button>
          </div>
